...
`
	...
	dropUsage = `drop [-f]    Drop everything inside file
...
File drivers: `+strings.Join(database.List(), ", ")+"\n", createUsage, gotoUsage, upUsage, downUsage, dropUsage, forceUsage)
	...
	case "drop":
		...
		if !*forceDrop {
			log.Println("Are you sure you want to drop the entire file schema? [y/N]")
			var response string
			_, _ = fmt.Scanln(&response)
			response = strings.ToLower(strings.TrimSpace(response))

			if response == "y" {
				log.Println("Dropping the entire file schema")
			} else {
				log.fatal("Aborted dropping the entire file schema")
			}
		}
